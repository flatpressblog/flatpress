...
### Bugfixes
- Public URL detection now separates backend and browser-visible scheme/port on TLS-terminating proxies and avoids leaking an internal port such as `:80` into HTTPS URLs. ([#948](https://github.com/flatpressblog/flatpress/pull/948))
- The update check now exposes transport/HTTP diagnostics when outbound server-to-server access is blocked. ([#948](https://github.com/flatpressblog/flatpress/pull/948))
...
- PrettyURLs update to version 3.0.6
    - Automatic mode now requires positive rewrite evidence and falls back to HTTP Get when rewrite support cannot be proven; explicit Pretty mode remains available for NGINX. ([#948](https://github.com/flatpressblog/flatpress/pull/948))
...
- Admin area:
    ...
    -  In the text input area for entries and static pages, the pipe character is displayed as ``|`` instead of as an HTML entity (``&#124;``) as before. ([#803](https://github.com/flatpressblog/flatpress/pull/803))
...
- Reflected XSS and host-based URL poisoning / open redirect fixed. ([#830](https://github.com/flatpressblog/flatpress/pull/830), [#831](https://github.com/flatpressblog/flatpress/pull/831), [#832](https://github.com/flatpressblog/flatpress/pull/832), [#833](https://github.com/flatpressblog/flatpress/pull/833), [#836](https://github.com/flatpressblog/flatpress/pull/836))<br><sub><i>Note: FlatPress is no longer accessible via HTTP and HTTPS, but only via the URL stored in the configuration. If HTTPS is stored in the configuration, HTTP requests are redirected to HTTPS.</i></sub>
- Hardening against splitting HTTP responses using CRLF injection. ([#834](https://github.com/flatpressblog/flatpress/pull/834))
...
- Emoticons plugin: update to version 1.1.3
    - New filter that converts Markdown emoticons in entry titles and static page titles to utf-8 emoticons. ([#781](https://github.com/flatpressblog/flatpress/pull/781))
- PhotoSwipe plugin update to version 2.0.5
    - Outputting galleries to RSS feeds is possible again ([#809](https://github.com/flatpressblog/flatpress/pull/809))
