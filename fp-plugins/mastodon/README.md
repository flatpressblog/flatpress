...
- Mastodon video and audio attachments are currently not imported by this plugin.
- If the Mastodon server has media limits, extra images are skipped instead of breaking the whole synchronization.
- When the plugin already knows an image description, it sends that description during the initial Mastodon media upload.
- If only the image description changes later, the plugin can update the description on supported Mastodon servers without re-uploading the image.
...
- Are the images in a FlatPress entry and not only in comments?
- Are they referenced with `[img]` or `[gallery]`?
- Does the Mastodon server allow the number and type of uploaded images?
...
## FAQ
  ...
  **Q:** Why are videos and audio not imported?  
  **A:** The plugin currently focuses on text threads and image attachments. Video and audio handling is not part of the current import feature set.
