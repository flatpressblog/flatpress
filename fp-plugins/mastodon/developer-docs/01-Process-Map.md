...
| P4a | Notification reply hints            | Content sync with `old_thread_reply_check` and `read:notifications` | Polls mention notifications, directly imports mapped-parent replies, and spends the same old-thread budget on unresolved notification contexts. | last_remote_notification_id, comments_remote, old_thread_context_cursor        | Notification hint, nested reply and shared-budget tests.                                   |
