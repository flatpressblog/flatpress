...
| P2c | Visitor/authenticated comment Mastodon export grant (Visitor comment Mastodon opt-in) | Public comment form renders `{comment_mastodon}` while Mastodon comment export is possible                                                                                                       | Shows a short Fediverse publication notice and stores the visitor approval in plugin state; visitor opt-ins and comments that carry FlatPress's stored `LOGGEDIN` marker store a state grant; the grant is kept even before credentials are complete, while visitor comments without that marker are accepted locally but never exported as Mastodon replies. | comment_reply_optins, dirty_comments                                                    | Opt-in display/state/export-guard tests.                                                               |
...
For P2c, CommentCenter moderation is an explicit edge case: when a visitor sets the Mastodon opt-in but the comment is held for approval, `commentcenter_comment_logged` lets the Mastodon plugin store the `comment_reply_optins` grant immediately. Later admin approval then reaches `comment_saved`, queues the dirty comment from the stored grant, and still does not treat the active admin session itself as visitor consent.
