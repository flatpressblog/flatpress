...
| Visitor comment opt-in      | Local comment remains possible           | Optional `{comment_mastodon}` checkbox stores an explicit visitor approval in `comment_reply_optins`; CommentCenter moderation preserves that approval through `commentcenter_comment_logged` before later admin approval. Comments that carry FlatPress's stored `LOGGEDIN` marker receive an authenticated grant there as well. The grant is independent of current credentials so later sync can export after configuration is completed; visitor comments without that marker stay local and are not exported as Mastodon replies. |
