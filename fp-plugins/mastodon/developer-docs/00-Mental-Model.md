...
| Local delete                | Immediate tombstone plus later reconcile | A deleted exported `source=local` comment is tombstoned in the delete hook before deletion sync; the mapping remains until the owned Mastodon reply is deleted. Imported `source=remote` replies are tombstoned as local ignore decisions and are never deleted remotely.                                                                                                                                                                                                                                                                                                                |
...
| Comment/reply sync gate     | Entry sync remains active                | Optional `disable_comment_reply_sync` blocks FlatPress-comment export, Mastodon-reply import, notification/context reply import and reply deletion follow-up.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Visitor comment opt-in      | Local comment remains possible           | Optional `{comment_mastodon}` checkbox stores an explicit visitor approval in `comment_reply_optins`, remains checked when another validator redisplays the submitted form, and works with or without CommentCenter. Moderated grants carry only a portable pending-file basename; approval finalizes that marker, while rejection or a later bounded sync prune removes orphaned pending grants. Comments that carry FlatPress's stored `LOGGEDIN` marker receive an authenticated grant as well. Visitor comments without a grant stay local and are not exported as Mastodon replies. |
...
| Imported status source link | FlatPress entry footer BBCode            | Imported Mastodon entries append a `Status.url` source link as `[url=... target=_blank rel="nofollow noopener noreferrer"]Mastodon[/url]`, pointing to the single toot/status rather than the author profile.                                                                                                                                                                                                                                                                                                                                                                            |
| Imported reply author link  | FlatPress comment author URL output      | Imported Mastodon reply author links use the stored `account.url`; only external profile URLs receive `target="_blank"`; root-domain same-host URLs stay internal only for recognized FlatPress routes/assets.                                                                                                                                                                                                                                                                                                                                                                           |
...
flowchart LR
    ...
    CommentReplyGate[disable_comment_reply_sync option]
    API <--> FP
    OneWay --> State
    OneWay --> AdminUI
    OneWay -. blocks Mastodon-to-FlatPress writes .-> FP
    CommentReplyGate --> State
    CommentReplyGate --> AdminUI
    CommentReplyGate -. blocks only comment/reply paths .-> Notifications
    CommentReplyGate -. keeps entry sync active .-> API
...
For exported comments, the robust invariant is stronger: the `comment_deleted` hook must create `local_deleted_pending_remote_delete` immediately, `plugin_mastodon_import_remote_comment()` must recreate that protection defensively if a legacy/manual file deletion bypassed the hook, and one Mastodon reply ID may belong to exactly one local comment mapping. A normal partial content sync must therefore be safe even when the old comment shard was not part of its initial workset.
...
5. Read the corresponding state fields in `02-State-Model.md`.
6. Find the implementation functions in `03-Function-Process-Matrix.md`.
7. If Mastodon is contacted, check endpoint version and fallback behavior in `04-API-Compatibility.md`.
8. Before changing code, identify at least one existing regression test or add one.
