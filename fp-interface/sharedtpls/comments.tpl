...
<form id="commentform" method="post" action="{$flatpress.params.entry|link:comments_link}#commentform" enctype="multipart/form-data">
	...
	{else}
		<p>
			<input type="hidden" name="name" id="name" value="{$flatpress.user.userid}">
			<input type="hidden" name="email" id="email" value="{$flatpress.user.email}">
			<input type="hidden" name="url" id="url" value="{$flatpress.user.www}">
		</p>
	{/if}
	<fieldset>
		<legend>{$lang.comments.fieldset2}</legend>
		<p>
			<label for="content">{$lang.comments.comment}</label>
			<br>
			<textarea name="content" {$error.content|notempty:'class="field-error"'} id="content" rows="10" cols="74">{$values.content}</textarea>
		</p>
