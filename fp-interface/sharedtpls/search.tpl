...
<form id="search" method="get" action="{$smarty.const.BLOG_BASEURL}search.php" enctype="multipart/form-data">
	<fieldset>
		<legend>{$lang.search.fset1}</legend>
		<p>
			<label for="keywords">{$lang.search.keywords}</label>
			<br>
			...
			<label><input type="radio" name="stype" value="full" id="fulltext">{$lang.search.fulltext}</label>
		</p>
	</fieldset>
	<fieldset>
		<legend>{$lang.search.fset2}</legend>
	...
	<fieldset>
		<legend>{$lang.search.fset3}</legend>
