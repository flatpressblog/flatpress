<?php
$lang ['admin'] ['uploader'] ['default'] = array(
	'head' => 'Nahrání souborů',
	'descr' => 'Vybrat jeden nebo více souborů k nahrání.',
	'fset1' => 'Výběr souborů',
	'fset2' => 'Nahrát',
	'submit' => 'Nahrát',
	'uploader_some_failed' => 'Tento soubor nebyl z bezpečnostních nebo systémových důvodů nahrán:',
	'uploader_metadata_failed' => 'Soubor byl nahrán, ale metadata nemohla být odstraněna:',
	'uploader_drop' => 'Přetáhněte soubory sem',
	'uploader_browse_hint' => '...nebo kliknutím vyberte soubory',
	'uploader_drop_active' => 'Uvolněte pro přidání',
	'uploader_selected_count' => 'Vybráno %d souborů',
	'uploader_clear' => 'Zrušit výběr',
	'uploader_remove' => 'Odstranit výběr',
	'uploader_limit_files' => 'Maximum %d files per upload.',
	'uploader_limit_size' => 'Maximum total upload size: %s.'
);

$lang ['admin'] ['uploader'] ['default'] ['msgs'] = array(
	1 => 'Soubor(y) nahraný(é)',
	-1 => 'Při pokusu nahrát soubor(y) nastala chyba.',
	-2 => 'Nahrávání bylo serverem odmítnuto: celková velikost nahrávaného souboru překračuje post_max_size (%s).',
	-3 => 'Nahrávání bylo serverem odmítnuto, pravděpodobně z důvodu omezení velikosti nahrávaného souboru nebo počtu souborů. Nebyly přijaty žádné soubory.',
	-4 => 'Nebyly přijaty žádné soubory. Před nahráním vyberte jeden nebo více souborů.'
);

$lang ['admin'] ['uploader'] ['browse'] = array(
	'head' => 'Procházet',
	'descr' => 'Vybrat jeden nebo více souborů k nahrání.',
	'fset1' => 'Výběr souborů',
	'submit' => 'Nahrát'
);
?>
