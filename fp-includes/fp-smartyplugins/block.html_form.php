...
function smarty_block_html_form($params, $content, &$smarty) {
	...
	if (!isset($params ['action'])) {
		if (function_exists('fp_setup_migration_mode') && fp_setup_migration_mode()) {
			$params ['action'] = "admin.php?p=" . ADMIN_PANEL . '&amp;action=' . ADMIN_PANEL_ACTION;
		} else {
	...
	if (is_null($content)) {
		// FIRST RUN: Only return form start + hidden inputs!
		return $str . $nonce;
	}
