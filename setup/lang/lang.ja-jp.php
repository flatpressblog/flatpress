...
記事本文への装飾や書式の指定には [url=https://wiki.flatpress.org/doc:plugins:bbcode target=_blank rel=external]BBcode[/url] (bulletin board code) がデフォルトで用意されています。BBCodeなら簡単に記事へ装飾できます。よくある装飾用タグが用意されています。例えば［b］タグで [b]ボールド[/b](太字)、［i］タグで [i]イタリック[/i](斜体)、などです。

[quote]お気に入りの引用を表示するための[b]［quote］[/b]タグもあります。[/quote]

[code]また、［code］タグでは 整形済みテキストが等幅形式で表示されます。
   スペース(空白)を使ったコンテンツのインデントだってほら...
      対応しています! :)[/code]
...
いまお読みいただいてるサンプルはブログ記事ですが、[url=static.php?page=about]「自己紹介」[/url] は [b]固定ページ[/b]です。固定ページにはコメントができません。またブログ記事と同時には表示されません。

固定ページは広報や告知などに向いています。 また [b]サイトのトップ(優先表示する)ページ[/b] に指定することもできます。ひいてはブログサイトでないサイト運営も可能ということです。固定ページを「サイトのトップ(優先表示する)ページ」に指定するには [url=admin.php]管理者用ページ[/url] の [b]設定[/b] を開きます。


[h4]プラグイン[/h4]

FlatPressは非常に高いカスタマイズ性があり、[url=https://wiki.flatpress.org/doc:plugins:standard target=_blank rel=external]プラグイン[/url] で機能を拡張することができます。ちなみに BBCode自身もプラグインによる機能です。

FlatPressの隠された珠玉の機能を紹介するためサンプルページを作りました :) 
あなたの編集を待っている [b]固定ページ[/b] が2つあります:
[list]
[*][url=static.php?page=about]自己紹介[/url]
[*][url=static.php?page=menu]メニュー[/url] (ページ内のリンクがサイドバーにも反映されることにご注目ください。これぞ[b]BlockParserウィジェットプラグイン[/b]の魔法なのです。このウィジェットやその他の詳細については [url=https://wiki.flatpress.org/doc:faq target=_blank rel=external]FAQ[/url] を参照してください!)
[/list]

[b]PhotoSwipeプラグイン[/b] を使用すると、float="left" または float="right" で揃えられた単一の画像として、いとも簡単に画像を配置できます。
...
サイドバーには固定された部品が1つもありません。この記事欄周辺のサイドバー上にある部品はすべて、自由に配置でき、そのほとんどはカスタマイズ可能です。その機能によっては、管理者用ページに設定用パネルが用意されているものもあります。
...
FlatPress-Leggeroテーマでは、クラシックからモダンまで4つのスタイルを自由に使用できます。これらのテンプレートは独自のものを作成するための素晴らしい出発点となります。
...
[*]他のユーザから投稿された [url=https://wiki.flatpress.org/res:themes target=_blank rel=external]素晴らしいテーマ[/url] を入手しましょう!
